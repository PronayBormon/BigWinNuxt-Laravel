...
const submitForm = async () => {
    ...
    const res = await apiFetch('/api/add-country', {
        method: 'POST',
        body: formData
    });
...
const getCountryList = async () => {
    const res = await apiFetch('/api/get-country', {
        method: 'POST'
    });
    ...
    if (!res.ok) {
        $notyf.error("Failed to load countries.");
        return;
    }
...
const deletCountry = async (id) => {
    const res = await apiFetch(`/api/delete-country/${id}`, {
        method: 'DELETE'
    });
    ...
    const modalElement = document.getElementById(`deleteConfirm${id}`);
    if (modalElement) {
        const modalInstance = bootstrap.Modal.getInstance(modalElement);
        if (modalInstance) {
            modalInstance.hide();
