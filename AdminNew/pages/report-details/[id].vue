...
const addResult = async () => {
    ...
    try {
        const res = await apiFetch('/api/add-single-match-result', {
            method: 'POST',
            body: formData
        });
        ...
        if (errRes?.errors) {
            Object.values(errRes.errors).forEach(messages =>
...
const addPredict = async () => {
  ...
  try {
    const res = await apiFetch('/api/add-singlematch-predict', {
      method: 'POST',
      body: formData
    });
    const data = await res.json();
    ...
    const modalElement = document.getElementById('addManual');
    const modalInstance = bootstrap.Modal.getInstance(modalElement);
    modalInstance?.hide();
...
const BowlerWinner = async () => {
  try {
    ...
    const query = new URLSearchParams({ id: matchId });
    const res = await apiFetch(`/api/bowler-winner-users?${query}`, { method: 'GET' });
    const data = await res.json();
...
const BatsmanWinner = async () => {
  try {
    ...
    const query = new URLSearchParams({ id: matchId });
    const res = await apiFetch(`/api/batsman-winner-users?${query}`, { method: 'GET' });
    const data = await res.json();
...
const teamdata = async () => {
  try {
    ...
    const res = await apiFetch(`/api/match-details/${id}`, { method: 'GET' });
    const data = await res.json();
...
const predictUsers = async (page = 1) => {
  ...
  try {
    const res = await apiFetch(`/api/match-report-users?${query.toString()}`, {
      method: 'GET',
    });
    const data = await res.json();

    predictList.value = data.data;
