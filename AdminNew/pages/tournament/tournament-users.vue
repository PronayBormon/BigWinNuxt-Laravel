                        ...
                        <div class="modal-content">
                            <div class="modal-body">
                                ...
                                <div class="adduser_form">
                                    ...
                                    <h1>Add Prediction Result</h1>
                                    <div class="d-flex align-items-center jsutify-content-center my-3">
                                        <ul class="nav tabs_navigation nav-tabs" id="myTab" role="tablist">
                                            <li class="nav-item" role="presentation">
                                                <button class="nav-link active" id="Batting-tab" data-bs-toggle="tab"
                                                    data-bs-target="#result_semiFianl-pane" type="button" role="tab"
                                                    aria-controls="semiFianl-pane" aria-selected="true">Semi
                                                    Final</button>
                                            </li>
                                            <li class="nav-item" role="presentation">
                                                <button class="nav-link " id="final-tab" data-bs-toggle="tab"
                                                    data-bs-target="#result_final-pane" type="button" role="tab"
                                                    aria-controls="final-pane" aria-selected="false">Final</button>
                                            </li>
                                            <li class="nav-item" role="presentation">
                                                <button class="nav-link " id="Champion-tab" data-bs-toggle="tab"
                                                    data-bs-target="#result_Champion-pane" type="button" role="tab"
                                                    aria-controls="Champion-pane"
                                                    aria-selected="false">Champion</button>
                                            </li>
                                        </ul>
                                    ...
                                    <div class="tab-content mb-2" id="myTabContent">
                                        ...
                                        <!-- ======== Final ========  -->
                                        <div class="tab-pane fade" id="result_final-pane" role="tabpanel"
                                            aria-labelledby="final" tabindex="0">
                                            <form @submit.prevent="addFinalPredictionResult">

                                                <div class="form-group mb-3">
                                                    <label>Select Team One</label>
                                                    <select v-model="result_team_one" @change="teamOnePlayer"
                                                        class="form-control">
                                                        <option value="" disabled>Select Team</option>
                                                        <option v-for="item in tournament.teams" :value="item.id">{{
                                                            item.team.name }}</option>
                                                ...
                                                <div class="form-group mb-3">
                                                    <label>Select Team Two</label>
                                                    <select v-model="result_team_two" @change="teamOnePlayer"
                                                        class="form-control">
                                                        <option value="" disabled>Select Team</option>
                                                        <option v-for="item in tournament.teams" :value="item.id">{{
                                                            item.team.name }}</option>
                                        ...
                                        <!-- ======== Champion ========  -->
                                        <div class="tab-pane fade " id="result_Champion-pane" role="tabpanel"
                                            aria-labelledby="Champion" tabindex="0">
                                            <form @submit.prevent="addChampionresult">

                                                <div class="form-group mb-3">
                                                    <label>Select Team</label>
                                                    <select v-model="result_champion_team" @change="teamOnePlayer"
                                                        class="form-control">
                                                        <option value="" disabled>Select Team</option>
                                                        <option v-for="item in tournament.teams" :value="item.id">{{
                                                            item.team.name }}</option>
...
// const fianl_user_id = ref("");
const result_team_one = ref("");
const result_team_two = ref("");
const result_hwt = ref("");
const result_hs = ref("");
...
const result_champion_team = ref("");
const result_mom = ref("");
const result_mot = ref("");
...
const result_teams = ref([
    { team_id: '', match: '', win: '', los: '', tie: '', pts: '' },
    { team_id: '', match: '', win: '', los: '', tie: '', pts: '' },
    { team_id: '', match: '', win: '', los: '', tie: '', pts: '' },
    { team_id: '', match: '', win: '', los: '', tie: '', pts: '' }
]);
const handleFormSubmit = async (url, formData, modalId, onSuccessCallback) => {
  try {
...
const addChampionresult = () => {
  const formData = new FormData();
  formData.append('match_id', tid);
  formData.append('team_id', result_champion_team.value);
  formData.append('mom', result_mom.value);
  formData.append('mot', result_mot.value);

  handleFormSubmit('/api/result/champion', formData, 'result');
};
...
const addChampion = () => {
  ...
  handleFormSubmit('/api/predictions/champion', formData, 'addManual', () => predictList(1));
};

const addFinalPredictionResult = () => {
  const formData = new FormData();
  formData.append('match_id', tid);
...
const addFinalPrediction = () => {
  ...
  handleFormSubmit('/api/predictions/final', formData, 'addManual', () => predictList(1));
};
...
const addSemiFinalPrediction = () => {
  ...
  handleFormSubmit('/api/predictions/semi-final', formData, 'addManual', () => predictList(1));
};

const addSemiFinalPredictionResult = () => {
  const formData = new FormData();
  formData.append('match_id', tid);
  formData.append('teams', JSON.stringify(result_teams.value));

  handleFormSubmit('/api/result/semi-final', formData, 'result');
};

const teamOnePlayer = async () => {
  try {
    const query = new URLSearchParams({ match_id: tid });
    const response = await apiFetch(`/api/tournamet/team-players?${query}`, { method: 'GET' });
    const data = await response.json();
    teamPlayerList.value = data;
  } catch (error) {
    await handleError(error);
  }
};
...
const details = async () => {
  try {
    const response = await apiFetch(`/api/tournament-details/${tid}`, { method: 'GET' });
    const data = await response.json();
...
const userList = async () => {
  try {
    const response = await apiFetch('/api/user-data', { method: 'GET' });
    const data = await response.json();
    userdata.value = data;
  } catch (error) {
    await handleError(error);
...
const predictList = async () => {
  try {
    const query = new URLSearchParams({ match_id: tid });
    const response = await apiFetch(`/api/users-with-predictions?${query}`, { method: 'GET' });
    const data = await response.json();
    PredictUsers.value = data;
  } catch (error) {
    await handleError(error);
  }
};

const tournamentWinner = async () => {
  try {
    const query = new URLSearchParams({ match_id: tid });
    const response = await apiFetch(`/api/tournament-winners?${query}`, { method: 'GET' });
    const data = await response.json();
