...
const updateMatch = async () => {
  ...
  try {
    const response = await apiFetch('/api/update-match', {
      method: 'POST',
      body: formData,
    });
...
const getdetails = async (id) => {
  try {
    const response = await apiFetch(`/api/match-details/${id}`, { method: 'GET' });
    const data = await response.json();

    editteamA.value = data.team_a.id;
    editteamB.value = data.team_b.id;
    editdateTime.value = data.time;
    editenddateTime.value = data.end_date;
...
const getTeamList = async () => {
  try {
    const response = await apiFetch('/api/getteamlist', { method: 'GET' });
    const data = await response.json();
    teamList.value = data;
  } catch (error) {
    await handleError(error);
...
const getMatchList = async (pages) => {
  try {
    ...
    const response = await apiFetch(`/api/get-matchList?${query.toString()}`, { method: 'GET' });
    const data = await response.json();
